...
public class HandlerPlayOutSpawnEntityLiving extends AbstractPlayOutEntitySpawn {
		...
		if(livingEntity != null && DisguiseManager.isDisguisedTo(livingEntity, observer)) {
			...
			if(DisguiseManager.getDisguise(livingEntity).getType().equals(DisguiseType.PLAYER) && !PacketHandler.modifyPlayerListEntry) {
				...
				Bukkit.getScheduler().runTaskLater(iDisguise.getInstance(), () -> {
					try {
						this.sendPacket(observer, playerInfoRemovePacket);
					} catch(Exception ex) {
				...
				if(DisguiseManager.getDisguise(livingEntity) instanceof FallingBlockDisguise && ((FallingBlockDisguise)DisguiseManager.getDisguise(livingEntity)).onlyBlockCoordinates()) {
					spawnPackets.get(0).getDoubles().write(0, Math.floor(livingEntity.getLocation().getX()) + 0.5);
					spawnPackets.get(0).getDoubles().write(1, Math.floor(livingEntity.getLocation().getY()));
					spawnPackets.get(0).getDoubles().write(2, Math.floor(livingEntity.getLocation().getZ()) + 0.5);
				}
	...
	private List<PacketContainer> getSpawnPackets(LivingEntity livingEntity, PacketContainer livingPacket) {
		try {
			...
			if(disguise instanceof MobDisguise) {
				...
				livingPacket.getIntegers().write(0, entity.getId());
				livingPacket.getUUIDs().write(0, entity.getUniqueID());
				livingPacket.getIntegers().write(1, EntityTypes.b.a(entity.getClass()));
				livingPacket.getDoubles().write(0, entity.locX);
				livingPacket.getDoubles().write(1, entity.locY);
				livingPacket.getDoubles().write(2, entity.locZ);
				livingPacket.getBytes().write(0, (byte)(int)(entity.yaw * 256.0F / 360.0F));
				livingPacket.getBytes().write(1, (byte)(int)(entity.pitch * 256.0F / 360.0F));
				livingPacket.getBytes().write(2, (byte)(int)(entity.aP * 256.0F / 360.0F));
				...
				livingPacket.getIntegers().write(2, ((int)(d2 * 8000.0D)));
				livingPacket.getIntegers().write(3, ((int)(d3 * 8000.0D)));
				livingPacket.getIntegers().write(4, ((int)(d4 * 8000.0D)));
				livingPacket.getDataWatcherModifier().write(0, WrappedDataWatcher.getEntityWatcher(entity.getBukkitEntity()));
